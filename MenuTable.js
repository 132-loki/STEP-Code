...
createMenuNutritionAsset(result,"report");
                    ...
                    if (dishArr[i].getObjectType().getID() == "DishRecipe") {
                        if (currSM.getValue("ShowGCWithRecipe").getSimpleValue() == "No") {
                            ...
                            if (showDishANData == "Yes") {
                                //Print Dish Recipes
                                htmlContent = htmlContent + getCoreDishContent(currSM, dishArr[i], "","report");
                            } else {
                                htmlContent = htmlContent + getIngSRContent(currSM, dishArr[i],"report");
                            }
                        } else {
                            var showDishANData = dishArr[i].getValue("ShowAN_From_DishRecipe").getSimpleValue();
                            if (showDishANData == "Yes") {
                            ...
                            while (currDishGCSRRefs.hasNext()) {
                                ...
                                if (currDishGCSRRef.getValue("IncludeGCSRinDigMenu").getSimpleValue() != "No") {
                                    if (!dishGCSRUnique.includes(currDishGCSRID)) {
                                        ...
                                        htmlContent = htmlContent + getCoreDishContent(currSM, currDishGCSR, dishArr[i],"report");
                                    }
                                }
                            }
                        }
                        ...
                        if (dishArr[i].getObjectType().getID() == "DishRecipe") {
                            ...
                            while (currDishGCSRRefs.hasNext()) {
                                ...
                                if (currDishGCSRRef.getValue("IncludeGCSRinDigMenu").getSimpleValue() != "No") {
                                    ...
                                    if (!dishGCSRUnique.includes(currDishGCSRID)) {
                                        dishGCSRUnique.push(currDishGCSRID);
                                        htmlContent = htmlContent + getCoreDishContent(currSM, currDishGCSR, currSM.getValue("SMGCName").getSimpleValue(),"report");
                                    }
                                }
                            }
                        }
...
function createMenuNutritionAsset(result,type)
{
	...
	var menuNutritionName=node.getName()+" - Menu Nutrition Report 1";
	var menuNutrition=step.getAssetHome().getAssetByID(menuNutritionID);
...
function getCoreDishContent(currSM, item, itemDish,reporttype) {
    var keyItemDetails = '';
	
	//var reportIndex = currSM.indexOf("report");
	////var reporttype = currSM.substring(reportIndex);
//var currSM = currSM.substring(0, reportIndex);

	
	//logger.info("currSM :"+typeof(currSM));
...
function getIngSRContent(currSM, item,reporttype) {
    var keyItemDetails = '';
    var subMenuName = currSM.getName();
